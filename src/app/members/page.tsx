...
import { Users,  SearchX, MapPin } from 'lucide-react'
...
export default async function MembersPage({
  ...
  try {
    ...
    members = membersRes.docs as unknown as Member[]
    totalPages = membersRes.totalPages
    currentPage = membersRes.page || 1
    totalDocs = membersRes.totalDocs

    const catRes = await payload.find({
        collection: 'member_categories',
    })
    categories = catRes.docs as unknown as MemberCategory[]
    ...
    console.error('Failed to fetch members page data:', error)
  }
      ...
      <header className="bg-mangogreen text-white shadow-md sticky top-20 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            <h1 className="text-xl font-bold flex items-center gap-2">
                <Users className="w-6 h-6 text-mangoyellow" />
                Member Directory
            </h1>
            <nav className="flex space-x-6 text-sm font-medium">
                <Link href="/statistics" className="hover:text-mangoyellow transition-colors font-bold">Market Data</Link>
                <Link href="/resources" className="hover:text-mangoyellow transition-colors font-bold">GAP Standards</Link>
            </nav>
        </div>
      </header>
              ...
              {members.map((member) => (
                <article key={member.id} className="member-card bg-white border border-slate-200 rounded-xl p-5 transition-all duration-300">
                  ...
                  <div className="flex items-center text-slate-500 text-sm mb-4 font-medium">
                    <MapPin className="w-4 h-4 mr-1 text-mangoyellow" />
                    ...
                    <ul className="flex items-center space-x-2">
                        <li>
                            <Link
                                ...
                                className="px-3 py-1 rounded border border-slate-300 text-slate-500 hover:bg-mangogreen hover:text-white transition-colors font-bold"
                            >
                                Prev
                            </Link>
                        </li>
