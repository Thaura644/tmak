          ...
          <div className="hidden md:flex space-x-8 items-center text-sm font-semibold uppercase tracking-wider text-slate-600">
            ...
            {session ? (
              <div className="flex items-center gap-4">
                <Link href="/dashboard" className="text-tmak-green font-bold hover:underline transition">Dashboard</Link>
                <button
                  onClick={() => signOut({ callbackUrl: '/' })}
                  className="bg-slate-100 text-slate-600 px-4 py-2 rounded shadow-sm hover:bg-slate-200 transition"
                >
                  Logout
                </button>
              </div>
